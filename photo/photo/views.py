...
def photo_post(request):
    ...
    if request.method == "POST":
        # 사용자 입력값 가져와서 photoform에 바인딩 시켜줌
        form = PhotoForm(request.POST)

        if form.is_valid(): # 유효성 검사
            # 입력값 데이터 베이스에 저장( 모델 폼인 경우 form.save() 하게되면 모델에 변화를 주게 됨 )
            form.save()
...
def photo_detail(request, pk):
    # pk에 해당하는 게시물 가져오기
    # get_object_or_404() : 특정 조건에 맞는 레코드가 존재한다면 객체 생성 or 존재하지 않는다면 404
